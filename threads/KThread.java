...
public class KThread {
	...
	public static void finish() {
		...
		//When a thread is finished, we have to set all the threads in its wait 
		//queue back to running, because they were all halted because of the join()
		KThread waitThread;
	...
	public void join() {
		...
		//System.out.println("Currently joining thread " + toString() + " to " + currentThread);

		//Check status for this thread, if it is not finished, wait till it finishes
		//System.out.println("Current Thread Status: " + this.status);
		if(this.status != statusFinished){
			//System.out.println(toString() + " status != statusFinishe so let currentThread wait for the access");
			//Put currentThread to the waitQueue and put it to sleep
			this.waitForJoin.waitForAccess(currentThread);
			KThread.sleep();							
		}

		//System.out.println(this + "successfully joined to " + currentThread.toString());
		Machine.interrupt().restore(intStatus);
	}
	...
	public static void selfTest() {
		...
		for(int i = 0; i < testThreads.length; i++){
			testThreads[i] = new KThread(new PingTest(i)).setName("forked thread " + i);
			testThreads[i].fork();
			System.out.println("Test thread " + i + " is running");
		}
		for(int i = 0; i < testThreads.length;i++){
			testThreads[i].join();
		}
	...
	public static KThread[] testThreads = new KThread[4];//For debugging purpose
